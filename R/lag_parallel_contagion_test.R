...
#'
lagParallelContagionTest <- function(df, iterations, cores = 1, difference = TRUE, threshold = 0.1, lagWin = 1){
  require(foreach)
  ...
  results <- foreach(n = 1:iterations, .combine = "rbind") %dopar% {
    if(difference == TRUE){
      ...
      if(x$statistic$p.value < threshold){
        ...
        j1mean.t <- j1mean[(lagWin+1):(length(j1mean)-lagWin)]
        j2mean.t <- j2mean[(lagWin+1):(length(j2mean)-lagWin)]

        j1mean.tm1 <- sapply(1:length(j1mean.t - lagWin), function(i) {
          mean(j1mean[i:(lagWin+(i-1))])
        })
        j2mean.tm1 <- sapply(1:length(j2mean.t - lagWin), function(i) {
          mean(j2mean[i:(lagWin+(i-1))])
        })
        ...
        #j1mean.lag <- j1mean[lagWin:(length(j1mean)-lagWin)]
